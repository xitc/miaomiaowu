...
func TestNormalSubscriptionReturnsFreshNodesOnFirstRefresh(t *testing.T) {
	for _, minutes := range []int{0, 5} {
		for _, useTemplate := range []bool{false, true} {
			t.Run(fmt.Sprintf("cache_%d_template_%t", minutes, useTemplate), func(t *testing.T) {
				...
				if useTemplate {
					...
					templateName = "fresh-template.yaml"
					if err := os.WriteFile(filepath.Join("rule_templates", templateName), []byte("proxies: []\nproxy-groups: []\nrules: [MATCH,DIRECT]\n"), 0600); err != nil {
						t.Fatal(err)
					}
				}
				...
				file, err := repo.CreateSubscribeFile(ctx, storage.SubscribeFile{Name: "fresh", TemplateFilename: templateName, Type: storage.SubscribeTypeCreate, Filename: filename, NormalLinkEnabled: true, DefaultOutputMode: storage.OutputModeNormal, SelectedTags: []string{"Provider/fresh-source"}})
				if err != nil {
					t.Fatal(err)
				}
